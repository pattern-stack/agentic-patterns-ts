...
7. **`abortSignal`** — **shipped.** `RunOptions.abortSignal` is forwarded to every provider call — `run()`, both of `runStructured()`'s single-call paths and its tier-2 fallback, and `stream()` (#504) — so a cancel interrupts an in-flight model call rather than burning tokens until it returns. The cooperative guards are per-method: `run()` and `stream()` check the signal between iterations, and `stream()` additionally before each tool dispatch. On an in-flight abort, `run()` returns `finishReason: "cancelled"` (no `agent.error`) and `runStructured()` emits a terminal `message.complete {finishReason:"cancelled"}` then throws `RunCancelledError`, never a raw `AbortError`. **Since #521:** when `RunOptions.timeout.runMs` is also set, every guard and provider call observes an EFFECTIVE signal — this `abortSignal` composed via `AbortSignal.any` with a derived `AbortSignal.timeout(runMs)` — never `abortSignal` alone; the resulting `finishReason` is discriminated by signal identity (an explicit `abortSignal` cancel always wins over a `runMs` deadline when both have fired), so `run()` can now also return `finishReason: "timeout"`. `stream()`'s mapping is unchanged (`agent.conversation.end {reason:"cancelled"}` either way — no new event vocabulary there). See `packages/agent-runtime/README.md`'s Known-limitations section for `modelMs`/`toolMs`'s own caveats.
