...
7. **`abortSignal`** — **shipped.** `RunOptions.abortSignal` is forwarded to every provider call — `run()`, both of `runStructured()`'s single-call paths and its tier-2 fallback, and `stream()` (#504) — so a cancel interrupts an in-flight model call rather than burning tokens until it returns. The cooperative guards are per-method: `run()` and `stream()` check the signal between iterations, and `stream()` additionally before each tool dispatch. On an in-flight abort, `run()` returns `finishReason: "cancelled"` (no `agent.error`) and `runStructured()` emits a terminal `message.complete {finishReason:"cancelled"}` then throws `RunCancelledError`, never a raw `AbortError`.

**Net:** `AgentRunner` uses the SDK correctly for the single-step-with-intercepted-tool-loop pattern. `reasoning` events were never actually missing (item 3), and the `providerOptions`/`reasoningEffort` passthrough now ships too (item 2, #514) — the factory just needs to pick the right resolved model.
...
- A harness whose probe does not report `features.structuredOutput` fails before any event with `HarnessStartError` code `"capability-missing"`.
...
- `RunOptions.abortSignal` mirrors `AgentRunner`: a signal already fired at entry throws `RunCancelledError` with no events; a signal that fires after `agent.message.start` but before the subprocess launches (during the probe) finalizes the open run with `agent.message.complete {finishReason:"cancelled"}` and then throws; a mid-run abort tears the session down, emits `agent.message.complete {finishReason:"cancelled"}` with the accrued content/tokens (not `""` — the harness family's D5 posture: partial output the user already saw is real, not discarded), then throws `RunCancelledError`.
