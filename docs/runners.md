...
3. **Reasoning deltas** — **already shipped**, not a gap: `fullStream` emits `reasoning` parts for extended-thinking models, and `AgentRunner.stream()` already turns them into `agent.reasoning` events (`agent-runner.ts:2123`/`:2158`/`:2171`/`:2319`); `run()` emits `agent.reasoning` too (`:958`). This item's earlier claim that the switch statement "drops them on the floor" was stale before #514 landed and is corrected here rather than left to mislead.
