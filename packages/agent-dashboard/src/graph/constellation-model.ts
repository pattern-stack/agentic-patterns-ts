...
export function computeFrame(steps: TraceStep[], cursor: number, graph: Constellation): Frame {
  ...
  const agentNodes = graph.nodes.filter(
    (n) => n.data.kind === "agent" || n.data.kind === "subagent",
  );
  ...
  if (activeStep) {
    if ((activeStep.kind === "tool_call" || activeStep.kind === "tool_result") && activeStep.tool) {
      ...
      activeNodeId =
        tn?.id ?? agentNodes.find((n) => n.data.label === activeStep.agent)?.id ?? null;
    } else {
      activeNodeId =
        agentNodes.find((n) => n.data.label === activeStep.agent)?.id ?? agentNodes[0]?.id ?? null;
    }
  }
