...
import { applyParts, eventsToAssistantMessage, type Part } from './model';
...
describe('applyParts', () => {
  ...
  test('pairs tool.start with tool.end on the same tool_call_id', () => {
    const parts = fold([
      { type: 'tool.start', tool_call_id: 't1', tool_name: 'searchDeals', arguments: { q: 'acme' } },
      { type: 'tool.end', tool_call_id: 't1', tool_name: 'searchDeals', result: [{ id: 1 }], duration_ms: 42 },
    ]);
  ...
  test('error event appends an error part (snake_case error_type)', () => {
    const parts = fold([{ type: 'error', message: 'kaboom', error_type: 'ModelError', recoverable: false }]);
