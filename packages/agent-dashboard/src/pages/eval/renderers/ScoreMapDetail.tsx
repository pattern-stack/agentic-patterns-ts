 ...
 * ~45 axes) grouped into readable buckets with threshold-colored meters. Unknown
 * axes fall into an "Other axes" catch-all so new metrics render for free.
 *
 * Payload: `{ kind: "score-map", axes: Record<string, number | null> }`.
 * Non-object `axes` ⇒ `null` (fall back to the raw-JSON expander).
 */
...
export const ScoreMapDetail: DetailRenderer = ({ detail }) => {
  const axes = (detail as { axes?: unknown }).axes;
