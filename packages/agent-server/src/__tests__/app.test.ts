...
describe("GET /agents", () => {
  ...
  it("returns agent summaries", async () => {
    const app = createApp({ agents: [testRegistration] });
    const res = await app.request("/agents");
    expect(res.status).toBe(200);
  ...
  it("defaults description to empty string", async () => {
    ...
    const app = createApp({ agents: [reg] });
    const res = await app.request("/agents");
...
describe("admin routes without adminService", () => {
  it("returns 501 for GET /admin/dashboard", async () => {
    const app = createApp({ agents: [] });
    const res = await app.request("/admin/dashboard");
    expect(res.status).toBe(501);
  });

  it("returns 501 for GET /admin/agents", async () => {
    const app = createApp({ agents: [] });
    const res = await app.request("/admin/agents");
    expect(res.status).toBe(501);
  });

  it("returns 501 for GET /admin/tools", async () => {
    const app = createApp({ agents: [] });
    const res = await app.request("/admin/tools");
    expect(res.status).toBe(501);
  });

  it("returns 501 for GET /admin/tokens", async () => {
    const app = createApp({ agents: [] });
    const res = await app.request("/admin/tokens");
    expect(res.status).toBe(501);
  });

  it("returns 501 for GET /admin/events/stream without eventBus", async () => {
    const app = createApp({ agents: [] });
...
describe("admin routes with adminService", () => {
  ...
  it("returns dashboard stats", async () => {
    const app = createApp({ agents: [], adminService: mockAdminService });
    const res = await app.request("/admin/dashboard");
    expect(res.status).toBe(200);
