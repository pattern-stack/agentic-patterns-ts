...
```mermaid
graph LR
  subgraph existing modules — symbols stay put
    CS["conversation/store.ts<br/>ConversationStore (was :59 ConversationStoreProtocol)<br/>InMemoryConversationStore (was :94 MemoryStore)"]
    ES["storage/event-store.ts<br/>EventStore (:94)"]
    SP["workflows/slot.ts<br/>Scratchpad (:60) — fork/join stays here"]
  end
  ST["stores/index.ts (NEW)<br/>curated family barrel<br/>+ ScratchpadStore type alias<br/>+ (#117 adds RunStore later)"]
  IDX["src/index.ts<br/>+ export * from './stores/index.js'"]
  CS --> ST
  ES --> ST
...
- **Version-bump placement**: this spec puts `0.7.0 → 0.8.0` in the impl PR (CI-verified safe —
  `check` passed on the bump-without-lock precedent `4562b66`). Alternative: keep the impl PR
  version-silent and bump in a separate `chore(release)` PR once #116/#117 land, so 0.8.0 ships
  the whole store-family stack at once. Human's call at Gate 1; default is bump-in-impl-PR.
- **`loadEventStore`** (`storage/load.ts`): deliberately NOT re-exported from `stores/` — it's a
  loader utility, not a store. Confirm at review if it should ride along.
