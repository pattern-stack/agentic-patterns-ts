...
### Re-check (rerun 1) — spec revision 4, HEAD `760cb3f`

**Target:** `git diff 7505cb5...HEAD -- ':!.ai-docs'` (branch `feat/cc-runner-run-structured` @ `760cb3f`; 20 files, +1242/-75)
**Against:** `.ai-docs/specs/547-cc-run-structured.md` (revision 4 — static sections Goal … Open questions)
**Verdict:** PASS_WITH_NOTES

**Blocker 1 — RESOLVED.** The §12 live case now builds a genuinely tool-less agent (`src/__tests__/claude-code-runner.test.ts:117-138`, `buildToolLessAgent()`: `RoleBuilder` with persona + default model only, no `.withCapability(…)`) and pins it with `expect(agent.getTools()).toHaveLength(0)` at `:229`. `buildAgentServers` iterates `agent.role.capabilities` (`sdk-bridge.ts:118`), which is empty, so `_buildOptions`' `if (agent.role.capabilities.length > 0)` guard (`claude-code-runner.ts:294`) never assigns `mcpServers`/`allowedTools` — the rewritten comment at `:234-238` ("no MCP servers are wired either … a structured result here proves the CLI force-appends the carrier after `tools` resolution") is now true of its own setup. §12's `tools: []` cell is restored exactly. Fix option (a), the stronger of the two offered.

**Prior notes:**
- Note 1 (file header + class doc) — **RESOLVED**. `claude-code-runner.ts:13-16` (header: `_buildOptions` now names `outputFormat: { type: "json_schema" }` and the carrier bypass) and `:167-173` (class doc: the whole mechanism plus the `docs/runners.md` §3.5 pointer). §6's closing instruction is discharged.
- Note 2 (§ Implementation notes mis-stated the `outputFormat` placement) — **RESOLVED**. Spec `:470` now reads "placed right after the `includePartialMessages` block and BEFORE `applyNativeTools`", matching `claude-code-runner.ts:273-285`, and says the earlier wording was wrong.
- Note 3 (mutation table not independently re-run) — **CARRIED**, same reason: a full runtime test run was in flight from the lead and this pass may not mutate source. Re-verified instead that every named test still exists and passes after both fix rounds, including the two rows whose targets moved (`features.structuredOutput` → test 9 now asserts `code === "capability-missing"`, `:459`; `cancelledRef` → the guard now lives in `_drainToBus`, test 8 unchanged and green). New sub-point: base test **7b** (the post-`message.start` abort race) has no row in the mutation table — it is now the only guard in `runStructured()` without one.

**Prior nits — all four RESOLVED:** `recoverable: false` asserted in base tests #4 (`coding-agent-runner-structured.test.ts:259-261`) and #5 (`:291-296`); accrued tokens asserted on the cancelled `message.complete` in #8 (`:404-408`); `adapter.probeCalls === 0` asserted in #10 (`:451`) against a new `probeCalls` counter on `FakeAdapter` (`:108`); and `CCRunnerProbe.publicBuildOptions`' `outputSchema` parameter now has a caller — the `nativeTools: "all"` plumbing case at `claude-code-api-runner.test.ts:207-212`.

**760cb3f refactor — no drift.** Checked against the three things it could have broken:
1. `run()` is behaviourally identical. `_drainToBus` (`coding-agent-runner.ts:338-357`) is a verbatim lift of the old try/catch/finally — same `_drainSession` iteration, same per-event `translator.translate` → `bus.publish`, same `_emitError`-then-rethrow, same unconditional `session.close()` in `finally` — returning `cancelledRef.value`; `run():180-188` then takes the same `_emitCancelledRun(bus, startEvent, model, translator)` branch it took before. `stream()` deliberately keeps its own loop (it must `yield`), documented at `:331-337`. `coding-agent-runner-abort.test.ts` (7 cases, the `run()`/`stream()` abort contract) is green.
2. `runStructured()`'s two cancel paths still match §7. The post-start race branch (`:272-285`) now calls `_emitCancelledRun(bus, startEvent, model)` — which is what §7's code block literally specifies (spec `:147`) and which resolves to `_completeEvent(startEvent, EMPTY_CANCELLED_ACCOUNTING, model)` (`:452-456`), i.e. the same single `message.complete{cancelled, content:""}` the pre-refactor inline call published. The mid-run branch (`:293-302`) calls `_emitCancelledRun(…, translator)`, which builds `{ ...translator.finalize(), finishReason: "cancelled" }` — byte-for-byte the accounting §7's inline `_completeEvent` built — publishes one event and returns a `RunResult` that is discarded, then throws `RunCancelledError`. No extra events on either path; tests 7b (`:369-399`) and 8 assert the exact event sequence.
3. Spec rev-4 static text and the code agree on `"capability-missing"`. Spec `:34` (the `HarnessStartError` code union), `:131` (§7's throw) and Tests #9 (`:279`) all say `"capability-missing"`; `coding-agent-runner.ts:507-511` throws it, `harness/types.ts:242-250` defines it on the new `HarnessStartErrorCode` union with a doc comment naming `runStructured`, the class doc (`:252-257`) now admits `CodingAgentRunner._startRun` as a throw site, and test 9 asserts the code. `docs/runners.md:195` carries the same statement.

**Declared deviations — both still stand.** (1) `disableSandbox: true` on the live runner: `claude-code-api-runner.ts` is untouched by both fix commits, so `disableSandbox` still resolves to `config: { mode: "host" }` and the constructor still pins `nativeTools: "none"` — the `tools: []` cell §12 targets is preserved, and the tool-less agent strengthens rather than weakens the justification. (2) No `// illustration` marker: `grep -c illustration docs/runners.md` → 0, so the house-style claim still holds after 760cb3f's §3.5 edits.

**Gates re-run here (read-only):** `bun run --filter=@pattern-stack/agentic-runtime typecheck` → 0 errors. `test src/runner/ src/runner/harness/` → **24 files, 408 tests, 0 failed** (includes all six touched/added files, 82 tests, plus the pre-existing `run()`/`stream()` abort and harness-contract suites). `biome check packages/agent-runtime/src/runner packages/agent-runtime/src/__tests__` → clean, 54 files. Not run: `build`, `bun install`, `check:dist-contract` (per mission); `src/__tests__/claude-code-runner.test.ts` is live-only and fails under root in this container (environmental, pre-existing).

**Blockers (0):**
- _None._

**Notes (3):**
- [carried] [mutation table, 11 rows + missing 7b row] Not independently re-run — see Prior notes above. Residual risk low: every named test exists, is substantively asserted, and passes after both fix rounds. Add a row for base test 7b, and spot-check two rows at validate.
- [`.ai-docs/specs/547-cc-run-structured.md:11` vs `:66`, `:153-160`, `:217-232`, `:260`, `§ Tests`] The rev-4 banner claims five quality notes were folded into the static sections; only two actually were. Landed: `capability-missing` (`:34`/`:131`/`:279`) and the post-start abort race (§7's code-block comment, `:146`). Still absent: the `_drainToBus` extraction (§7's "implement verbatim" block at `:153-160` still shows the inline drain loop and the inline cancelled `_completeEvent`, both of which the code no longer has), base test 7b (§ Tests enumerates 1–12 only), `CC_STRUCTURED_OUTPUT_TOOL`'s re-export from `runner/index.ts` (§1's `runner/index.ts` bullet `:66` and § Interfaces `:260` unchanged), `FINISH_REASON_STRUCTURED_OUTPUT_RETRIES` (absent from §1 and § Interfaces), and `packages/agent-runtime/src/runner/harness/index.ts` — a file the diff touches (`export type { HarnessStartErrorCode }`) that the File-level plan `:217-232` does not list. Every one of these IS recorded in § Implementation notes' quality fix-round paragraph (`:502`), so this is a spec-trail gap, not undeclared work — but a reader who trusts § Approach + § File-level plan as the contract will find code the contract does not describe.
- [`packages/agent-runtime/src/runner/types.ts:235-240`] The public `RunOptions.abortSignal` JSDoc still says flatly that `runStructured()`'s "pre-start abort throws `RunCancelledError` with no events". This is verbatim what §8 asks for, so it is not a spec violation — but 760cb3f split that window in two everywhere else (`coding-agent-runner.ts:272-280` and `:536-540`; `docs/runners.md:196`), leaving the in-editor JSDoc as the one surviving description that a post-`message.start` cancel contradicts. One clause ("a signal that fires during the probe finalizes the open run first") would align all three.

**Nits (3):**
- [`packages/agent-runtime/src/runner/errors.ts:28-32`] `FINISH_REASON_STRUCTURED_OUTPUT_RETRIES` is a finishReason constant living in a file whose header (`:1-8`) declares itself "Shared runStructured() error classes"; the file doc was not widened. It is also the only export of that file not re-exported from `runner/index.ts:7`, so a consumer branching on `result.finishReason` still hardcodes the literal the constant exists to centralize.
- [`docs/runners.md:187`] The carrier paragraph now tells hosts to write `extraDisallowedTools: [CC_STRUCTURED_OUTPUT_TOOL]` but does not say where to import it from (`@pattern-stack/agentic-runtime` root, via `runner/index.ts:24`). One parenthetical would close it.
- [`packages/agent-runtime/src/__tests__/claude-code-runner.test.ts:234-236`] The live comment's two claims are now unevenly pinned: `agent.getTools()).toHaveLength(0)` (`:229`) covers "tool-less", but "no MCP servers are wired either" has no assertion — it follows from `sdk-bridge.ts:118` and the `capabilities.length > 0` guard, which the test cannot see. The existing `"SDK bridge converts capabilities to MCP server configs"` unit test covers only the with-capability direction.

**Reviewed by:** reviewer agent · 2026-09-15T05:07:00Z
