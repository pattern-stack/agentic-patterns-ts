...
## Rail v3 — the self-captioning Scratchpad rail (clarity pass, 2026-07-12)
...
- **Vocabulary**: tab `State` → **`Scratchpad`**; rail header **"Scratchpad · what this run carries between stages"** (the gloss carries the meaning; the title is the runtime's own word, `NodeRunContext.scratchpad`); density toggle relabels to `scratchpad:`. **Owner decision (2026-07-12): "Memory" is OFF the table** — reserved for true cross-session, user-specific memory; the design run's original "Memory" pick is superseded. Other rejected candidates: "State" (substrate word, failed live), "Carry" (names motion, not contents). The header tooltip explicitly disambiguates: "Not user memory: it lives and dies with the run."
