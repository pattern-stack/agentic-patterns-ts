...
- **agent-core**: the `src/protocols/` layer is removed (#490) — a project-tracker domain (sprints, tasks, tags, comments, documents, users, environments) sitting in the public API of an *agent framework*. It had exactly one consumer in this repo: the core barrel re-exporting it. 104 exports go, ~22% of core's root namespace. Removed protocol interfaces: `TaskProtocol`, `ProjectProtocol`, `TagProtocol`, `UserProtocol`, `SprintProtocol`, `CommentProtocol`, `DocumentProtocol`, `EnvironmentProtocol`. Removed schemas/types include the generically-named ones a downstream `import type` could plausibly have named — `Task`, `TaskSchema`, `Project`, `ProjectSchema`, `Sprint`, `SprintSchema`, `Tag`, `TagSchema`, `User`, `UserSchema`, `Team`, `TeamSchema`, `Document`, `DocumentSchema`, `Comment`, `CommentSchema`, `Reaction`, `ReactionSchema`, `Relation`, `RelationSchema`, `Environment`, `EnvironmentSchema`, `Priority`, `IssueType`, `StatusCategory`, `RelationType`, `WorkPhase`, `ProjectStatus`, `SprintStatus`, `DocType`, `TagGroup`, `UserType`, `UserRole`, `EnvironmentStatus`, `ResourceProfile`, `NetworkPolicy`, every `Create*Input`/`Update*Input`/`*Filter` and their `*Schema` counterparts, and the `Sandbox*` set. **Migration:** these were plain Zod schemas and interfaces with no framework coupling — copy the ones you use into your own codebase. No deprecation cycle: core is pre-1.0 and this is a re-release credibility item.

- **agent-core**: `ProtocolModel<T>` is removed (#490) — called out separately because it is different in kind from the schemas above. It was an exported **abstract base class that exists to be extended** (`atoms/base.ts`, re-exported from `atoms/index.ts`, documented in the package README), so "nothing extends it" is verified *in this repo only* and is unverifiable downstream. **Migration:** vendor it — it is ~5 lines over `AgenticModel`, which is unchanged and still exported:
...
- **agent-runtime**: `RunOptions.abortSignal` now reaches every provider call (#504) — `run()`'s `generateText`, `runStructured()`'s no-tools path, and its tier-2 fallback previously never received it (only `stream()` and the capable path did), so a cancel during a long model call kept burning tokens until the call returned. Abort rejections are normalized: `run()` closes the interrupted call with `agent.llm.end {finishReason:"cancelled"}` — no `agent.error` — and returns `finishReason: "cancelled"` (D1: never throws); `runStructured()` throws its existing `RunCancelledError` on any abort, never a raw `AbortError` — first emitting a terminal `agent.message.complete {finishReason:"cancelled"}` when the run had already started, so run-store exporters still finalize the row — including on the capable path, whose abort previously escaped raw with a spurious `agent.error`. Detection requires the caller's own signal to have fired AND either identity with the signal's `reason` (what `fetch` rejects with — covers `AbortSignal.timeout()`'s `TimeoutError` and `controller.abort(customReason)`) or an abort-shaped error name; a provider's unrelated abort-shaped error still routes through the normal error path.
